...
public class ShadowGimpy extends Configurable implements GimpyEngine {
    ...
    public BufferedImage getDistortedImage(BufferedImage baseImage) {
        ...
        shadowFilter.setRadius(0F);
        shadowFilter.setDistance(0F);
        shadowFilter.setOpacity(0F);

        RippleFilter rippleFilter = new RippleFilter();
        rippleFilter.setWaveType(0);
        rippleFilter.setXAmplitude(7.6F);
        rippleFilter.setYAmplitude(rand.nextFloat() + 1.0F);
        rippleFilter.setXWavelength((float)(rand.nextInt(10) + 10));
        rippleFilter.setYWavelength((float)(rand.nextInt(6) + 4));
