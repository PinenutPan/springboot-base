...
import com.sinosoft.cms.base.bean.MetaVo;
...
import com.sinosoft.cms.base.module.sys.mapper.SysMenuMapper;
...
public class SysMenuServiceImpl implements ISysMenuService {
    ...
    @Override
    public List<String> generateMenus(Object obj, String controllerName) {
        String remark = "";
        Date date = new Date();
        if (!controllerName.startsWith("job") && !controllerName.startsWith("sys")) {
            Annotation[] declaredAnnotations = obj.getClass().getDeclaredAnnotations();
            for (Annotation declaredAnnotation : declaredAnnotations) {
                if (declaredAnnotation instanceof Api) {
                    Api api = (Api) declaredAnnotation;
                    remark = api.tags()[0];
                } else {
                    remark = controllerName;
                }
            }
        }
        ...
        for (Method m : methods) {
            ...
            for (Annotation annotation : annotations) {
                ...
                if (annotation instanceof PreAuthorize) {
                    ...
                    perm = preAuthorize.hasPermi();
                }
            }
            if (StringUtils.isNotBlank(perm) && !"default".equals(menuName)) {
                SysMenu sysMenu = menuMapper.selectMenuByPerms(perm);
                if (null == sysMenu) {
                    sysMenu = new SysMenu();
                    sysMenu.setComponent(controllerName);
                    sysMenu.setPerms(perm);
...
//                menuMapper.insertMenu(sysMenu);
                    sysMenuList.add("菜单名：" + menuName + " 所属:" + controllerName + " 权限：" + perm);
                }
            }
        }
        return sysMenuList;
